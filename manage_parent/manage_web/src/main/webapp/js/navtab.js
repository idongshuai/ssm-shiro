	    ...
	    if(tabIndex === -1){
			...
			//监听关闭事件
			    ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
			    	element.tabDelete(ELEM.tabFilter, data.tabId).init();
			    });
		    };
		    //切换到当前打开的选项卡
		    element.tabChange(ELEM.tabFilter,data.tabId);
		    }else {
	    	    /**用于刷新当前iframe  2018/8/2**/
	    		$(".larry-iframe")[tabIndex].src="";
                $(".larry-iframe")[tabIndex].src=data.href;
			    element.tabChange(ELEM.tabFilter, data.tabId);
		    }
        };
