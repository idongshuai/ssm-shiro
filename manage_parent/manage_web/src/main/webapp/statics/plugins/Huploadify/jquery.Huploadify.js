		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					...
					inputStr += '"/>';
					inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="layui-btn">';
					inputStr += '<i class="fa fa-upload" aria-hidden="true"></i>&nbsp;'+option.buttonText;
					inputStr += '</a>';
					...
					returnObj =  {
						...
						disable : function(instanceID){
							...
							parent.find('.layui-btn').css('background-color','#888').off('click');
						},
						ennable : function(instanceID){
							//点击上传按钮时触发file的click事件
							var parent = instanceID ? $('file_upload_'+instanceID+'-button') : $('body');
						  	parent.find('.layui-btn').css('background-color','#707070').on('click',function(){
					...
					_this.find('.layui-btn').on('click',function(){
